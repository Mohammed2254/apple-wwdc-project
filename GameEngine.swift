...
final class GameEngine: ObservableObject {
    ...
    /// Fails with a descriptive assertionFailure in DEBUG; gracefully no-ops in RELEASE.
    private func loadContent() {
        guard let url = Bundle.main.url(forResource: "content", withExtension: "json") else {
            assertionFailure("[LifeQuest] content.json not found in bundle. Add it to Copy Bundle Resources.")
            return
        }
        do {
            ...
            assertionFailure("[LifeQuest] content.json decode error: \(error)")
        }
    }
